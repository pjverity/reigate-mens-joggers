...
<%--@elvariable id="eventCreationFormObject" type="uk.co.vhome.clubbed.web.site.organiser.EventCreationFormObject"--%>
<%--@elvariable id="eventCancellationFormObject" type="uk.co.vhome.clubbed.web.site.organiser.EventCancellationFormObject"--%>
				...
				<table class="table table-hover table-sm mb-0">
					...
					<c:forEach var="event" items="${completedEvents}">
						<tr onclick="gotoUrl('organiser/event-scheduling?eventId=${event.id}')" style="cursor: pointer">
							...
							<td nowrap><fmt:formatNumber type="number" maxFractionDigits="2" value="${event.eventDetails.distance}"/></td>
						</tr>
					</c:forEach>
					</tbody>
				</table>
			...
			<div class="card card-outline-success mt-2">
				...
				<ul class="list-group list-group-flush">
					<c:forEach var="user" items="${selectedEvent.userDetailsEntities}">
						<li class="list-group-item"><a href="mailto:${user.userEntity.username}">${user.firstName}&nbsp;${user.lastName}</a></li>
					</c:forEach>
				</ul>
				</c:if>
			</div>
